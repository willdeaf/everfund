...
import { ModalProps } from "./resources/types"
  ...
  private setupButtonListeners() {
    ...
    document.addEventListener(
      ...
      function (e: MouseEvent) {
        ...
        if (!match || that.donationWidgetOpen) return
        e.preventDefault()
        e.stopPropagation()
      ...
      function (e) {
        ...
        switch (e.data.message) {
          ...
          case "everfund:failure":
            that.onFailure(e.data.content)
            break
          case "everfund:close":
            embed && enableBodyScroll(embed)
            embed && embed.remove()
            that.donationWidgetOpen = false
            that.onClose()
            // clearAllBodyScrollLocks();
            break
        }
...
export default EverfundWidgetClient
